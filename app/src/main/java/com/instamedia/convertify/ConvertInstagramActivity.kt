...
class ConvertInstagramActivity : AppCompatActivity() {

    private val client = OkHttpClient()
    private val apiUrl = "${BuildConfig.API_BASE_URL}/instagram"
    private val apiKey = BuildConfig.API_KEY

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_convert_instagram)
    ...
    private fun enviarUrlAlServidor(instagramUrl: String) {
        val json = JSONObject()
        json.put("url", instagramUrl)
        ...
        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                runOnUiThread {
                    Toast.makeText(this@ConvertInstagramActivity, "Error de conexión", Toast.LENGTH_SHORT).show()
                }
                ...
                if (response.isSuccessful && responseBody != null) {
                    ...
                    if (videoUrl != null) {
                        runOnUiThread {
                            ...
                            runOnUiThread {
                                Toast.makeText(this@ConvertInstagramActivity, "Descarga iniciada", Toast.LENGTH_SHORT).show()
                            }
                        }
                    } else {
                        runOnUiThread {
                            Toast.makeText(this@ConvertInstagramActivity, "No se encontró el video", Toast.LENGTH_SHORT).show()
                        }
                    }
                } else {
                    runOnUiThread {
                        Toast.makeText(this@ConvertInstagramActivity, "Respuesta inválida del servidor", Toast.LENGTH_SHORT).show()
                    }
                }
            }
        })
